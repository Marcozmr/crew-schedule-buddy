...
export default function PerDiemPage() {
  ...
  const [form, setForm] = useState({ date: '', location: '', quantity: 1, unit_value: 0, notes: '' });
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    const total = form.quantity * form.unit_value;
    await supabase.from('perdiem_entries').insert({ user_id: user.id, date: form.date, location: form.location, quantity: form.quantity, unit_value: form.unit_value, total_value: total, notes: form.notes || null });
    toast.success('Diária adicionada!');
    setForm({ date: '', location: '', quantity: 1, unit_value: 0, notes: '' });
    setSaving(false);
    load();
  };
    ...
    <AppLayout>
      ...
      <div className="bg-card rounded-xl p-6 shadow-card mb-6 border border-border">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
          ...
          <div><Label className="text-xs">Quantidade</Label><Input type="number" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: parseFloat(e.target.value) || 0 }))} /></div>
          <div><Label className="text-xs">Valor Unitário</Label><Input type="number" value={form.unit_value} onChange={e => setForm(f => ({ ...f, unit_value: parseFloat(e.target.value) || 0 }))} /></div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">Total: <strong className="text-foreground">R$ {(form.quantity * form.unit_value).toFixed(2)}</strong></p>
