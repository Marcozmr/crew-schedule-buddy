      ...
      <div className="px-5 py-3 flex items-center justify-between border-b border-border/40">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">{analysis ? 'Análise da jornada atual' : 'Aguardando escala'}</p>
          </div>
        </div>
        <div className={`px-3 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wider ${meta.badge}`}>
          {analysis?.focus ? formatComplianceStatus(analysis.focus.status) : 'Sem informações'}
        </div>
      </div>
